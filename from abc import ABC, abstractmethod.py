...
class HDD(Menu):
    ...
    def create_file(self, name, type, size, unit):
        self.files.append(HDD(name, None, type, size, unit))

    def create_folder(self, name):
        self.files.append(HDD(name, self, "File folder", 0, ""))
...
def initialize_hdd():
    root = HDD("D")  
    root.create_file("Video", "mp4", 500, "GB")  
    root.create_file("Audio", "mp3", 100, "MB")  
    root.create_file("Text", "txt", 100, "KB")  
    root.create_folder("Files")  
    return root


open_menu(initialize_hdd())
